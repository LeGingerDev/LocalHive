-- Fix duplicate match_items_by_embedding functions
-- Drop all existing versions first
DROP FUNCTION IF EXISTS public.match_items_by_embedding(vector, integer, uuid[]);
DROP FUNCTION IF EXISTS public.match_items_by_embedding(vector, integer, uuid[], double precision);

-- Create the single, correct version with 3 parameters
