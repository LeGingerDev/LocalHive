...
serve(async (req) => {
  ...
  try {
    ...
    if (demoError || !demoData?.is_demo) {
      return new Response(
        JSON.stringify({ success: false, error: 'Demo mode is not enabled' }),
        { 
    ...
    const { data: { user }, error: authError } = await supabaseClient.auth.admin.getUserByEmail('demo@visu.app')

    if (authError || !user) {
      return new Response(
        JSON.stringify({ success: false, error: 'Demo user not found. Please run create_demo_account() first.' }),
        { 
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
          status: 400 
        }
