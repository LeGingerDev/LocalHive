...
interface GroupsScreenProps extends AppStackScreenProps<"Groups"> {}
...
/**
 * GroupsScreen - A defensive screen with proper error handling and loading states
 * 
 * Features:
 ...
 */
export const GroupsScreen: FC<GroupsScreenProps> = () => {
  // #region Private State Variables
  const [_isLoading, setIsLoading] = useState<boolean>(true)
  const [_data, setData] = useState<GroupsData | null>(null)
  const [_error, setError] = useState<GroupsError | null>(null)
  const [_isRefreshing, setIsRefreshing] = useState<boolean>(false)
  // #endregion

  // #region Hooks & Context
  const { themed } = useAppTheme()
  // const navigation = useNavigation<AppStackNavigationProp<"Groups">>()
  // #endregion
  ...
  const _fetchData = useCallback(async (): Promise<void> => {
    try {
      ...
      console.error("[GroupsScreen] Error fetching data:", error)
    } finally {
      setIsLoading(false)
  ...
  const _renderContent = (): JSX.Element => (
    ...
    >
      <Text style={themed($title)} text="Groups" />
      {_data && (
        <>
