...
const AuthPrompt = () => {
  ...
  return (
    <View style={themed($authPromptContainer)}>
      ...
      <Text style={themed($authPromptText)} text="You need to be signed in to view and manage your invitations." />
    </View>
  )
}
      ...
      {!user ? (
        ...
        >
          ...
          <View style={themed($section)}>
            <Text style={themed($sectionTitle)} text={`Pending Invitations (${pendingInvitations.length})`} />
          ...
          <View style={themed($section)}>
            <Text style={themed($sectionTitle)} text={`Sent Invitations (${sentInvitations.length})`} />
                ...
                <View key={invitation.id} style={themed($sentInvitationCard)}>
                  <View style={themed($sentInvitationInfo)}>
                    <View style={themed($avatar)}>
                      <Text style={themed($avatarInitial)} text={invitation.group?.name?.[0] || "G"} />
                    </View>
                    <View style={{ flex: 1 }}>
                      <Text style={themed($sentInvitationTitle)} text={invitation.group?.name || "Unknown Group"} />
                      <Text style={themed($sentInvitationMeta)} text={`Invited ${invitation.invitee?.full_name || invitation.invitee?.email || "Unknown User"}`} />
                      <Text style={themed($sentInvitationStatus)} text={`Status: ${invitation.status}`} />
                    </View>
                  </View>
...
const $sectionTitle = ({ typography, colors, spacing }: any): TextStyle => ({ fontFamily: typography.primary.medium, fontSize: 16, color: colors.text, marginBottom: spacing.sm })
...
const $sentInvitationCard = ({ colors, spacing }: any) => ({ backgroundColor: colors.background, borderRadius: 16, padding: spacing.lg, marginBottom: spacing.md, shadowColor: colors.palette.neutral800, shadowOpacity: 0.04, shadowRadius: 8, shadowOffset: { width: 0, height: 2 }, elevation: 1 })
...
const $sentInvitationTitle = ({ typography, colors }: any) => ({ fontFamily: typography.primary.medium, fontSize: 16, color: colors.text })
const $sentInvitationMeta = ({ typography, colors }: any) => ({ fontFamily: typography.primary.normal, fontSize: 13, color: colors.textDim, marginTop: 2 })
const $sentInvitationStatus = ({ typography, colors }: any) => ({ fontFamily: typography.primary.normal, fontSize: 12, color: colors.textDim, marginTop: 2 })
const $cancelButton = ({ colors, spacing }: any): ViewStyle => ({ backgroundColor: colors.error, borderRadius: 8, paddingVertical: spacing.xs, paddingHorizontal: spacing.md, alignSelf: "flex-start" as const, marginTop: spacing.sm })
const $cancelButtonText = ({ colors, typography }: any): TextStyle => ({ color: colors.background, fontFamily: typography.primary.medium, fontSize: 14, textAlign: "center" as const })