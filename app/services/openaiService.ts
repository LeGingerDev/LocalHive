  ...
  try {
    ...
    for (const [category, keywords] of Object.entries(categoryKeywords)) {
      if (keywords.some(keyword => questionLower.includes(keyword))) {
        ...
        if (categoryFiltered.length > 0) {
          ...
          console.log("[OpenAIService] Category filtered to", category, "items:", relevantItems.length)
          break
        }
      }
    }
    ...
    if (personMatch) {
      ...
      if (personFiltered.length > 0) {
        ...
        console.log("[OpenAIService] Person filtered to", personName, "items:", relevantItems.length)
      }
    }
