...
import { GroupInvitation, InvitationStatus, Profile, Group } from "@/services/api/types"
...
export class InvitationService {
   ...
   */
  static async getSentInvitations(): Promise<{
    data: GroupInvitation[] | null
    error: PostgrestError | null
  }> {
    try {
   ...
   */
  static async createInvitationByCode(
    groupId: string,
    personalCode: string,
    ...
    try {
      ...
      if (profileError || !inviteeProfile) {
        return {
          data: null,
          error: {
            message: "User not found with this invitation code",
            details: "",
            hint: "",
            code: "404",
          } as PostgrestError,
  ...
  static async cancelInvitation(invitationId: string): Promise<{ error: PostgrestError | null }> {
    try {
      ...
      if (invitationError || !invitation) {
        return {
          error: {
            message: "Invitation not found or you don't have permission to cancel it",
            details: "",
            hint: "",
            code: "404",
          } as PostgrestError,
    ...
    try {
      ...
      if (!membership) {
        return {
          data: null,
          error: {
            message: "You must be a member of this group to view invitation stats",
            details: "",
            hint: "",
            code: "403",
          } as PostgrestError,
