...
export const SearchModal: FC<SearchModalProps> = ({ visible, onClose, enableAI = false }) => {
  ...
  const [isAIMode, setIsAIMode] = useState(() => {
    ...
    return enableAI && subscription.isPro
  })
    ...
    (text: string) => {
      ...
      if (!isAIMode) {
        ...
        debounceTimeout.current = setTimeout(async () => {
          ...
          try {
            console.log("[SearchModal] Searching for:", text)
            const items = await searchItemsByVector(text)
            console.log("[SearchModal] Search results:", items)
            console.log("[SearchModal] Number of results:", items.length)
  ...
  const handleAISearch = useCallback(async () => {
    ...
    try {
      console.log("[SearchModal] AI Query:", query)
      const {
        data: { user },
      ...
      console.error("[SearchModal] AI error:", e)
  ...
  return (
    ...
    >
      ...
      >
        ...
        <View style={themed($searchInputArea)}>
...
const $searchInputArea: ThemedStyle<ViewStyle> = ({ colors, spacing }) => ({
  backgroundColor: colors.background,
  paddingHorizontal: spacing.md,
